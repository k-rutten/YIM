...
import { AccessControlSystem } from '@src/models/generated/registration';
...
export default class IloqPicker extends mixins(BaseMixin, FilterMixin, AuthorizationMixin, ProcessMixin)
{
    @Prop()
    variant: string;
    ...
    async onItemSearch(filter: PageRequest): Promise<void>
    {
        ...
        {
            case 'iLOQ access group':
                this.response = await registrationApiService.findAccessSystemAccessGroups(AccessControlSystem.Iloq, pageRequestFilters);
                break;
        }
