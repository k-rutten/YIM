...
@Component
export default class AuthorizationAccessOverview extends mixins(BaseMixin, FilterMixin, EnumLocalizerMixin, KendoTableMixin, ProcessMixin, AuthorizationMixin, FormatMixin)
{
    ...
    setDefaultColumns(): void
    {
        ...
        },
        {
            ...
            title: AppHost.i18n.global.t('Location') as string,
            headerClassName: getSortClass(),
    ...
    async loadAuthorizations(): Promise<void>
    {
        const params = new AuthorizationsFilter({
            ...
            authorizationsScope: AuthorizationsScope.AuthorizationAccess
        });
    ...
    async onRowClick($event: GridRowClickEvent): Promise<void>
    {
        this.$router.push(YimUrls.getAuthorizationAccessDetailUrl($event.dataItem.authorizationId));
    }
}
